...
function DockyardBackground() {
  ...
  useBgEffect(() => {
    ...
    let W = 0, H = 0, dpr = 1, raf = 0, tick = 0, flash = 0, focusBox = null;
    let cells = [], boxes = [], pulses = [], sp = 150;
    ...
    function inKeepout(x, y) { const k = keepout(); const dx = (x - k.cx) / k.rx, dy = (y - k.cy) / k.ry; return dx * dx + dy * dy < 1; }
    ...
    function parseIcon(svg) {
      ...
      svg.querySelectorAll('path,circle,line,polyline,polygon,rect,ellipse').forEach(el => {
        const g = el.tagName.toLowerCase(), n = (k) => parseFloat(el.getAttribute(k)) || 0;
    ...
    function drawIcon(ops, cx, cy, half, alpha, rgb) {
      ...
      for (const op of ops) {
        ...
        if (op.g === 'circle') ctx.arc(op.cx, op.cy, op.r, 0, 6.2832);
          ...
          if (ctx.roundRect) ctx.roundRect(op.x, op.y, op.w, op.h, r); else ctx.rect(op.x, op.y, op.w, op.h);
    ...
    function hasActivePulse(b) { for (const p of pulses) if (p.from === b || p.to === b) return true; return false; }
    function shutdown(b) { if (b && b.state === 'live' && !hasActivePulse(b)) b.state = 'down'; }

    function liveNeighbours(b) {
      const out = [];
    ...
    return () => {
      ...
      if (window.DockyardMesh === api) delete window.DockyardMesh;
    };
  }, []);

  return <canvas ref={ref} className="stage-canvas" aria-hidden="true" />;
}

Object.assign(window, { DockyardBackground });
