...
|1.15|Tighten budgets (`anyComponentStyle` 32→16KB; `initial` warn/error ceilings). **`inlineCritical` MUST stay `false`** under our CSP: Beasties rewrites the stylesheet `<link>` to `media="print" onload="this.media='all'"`, and that inline `onload` is governed by `script-src` (NOT `style-src 'unsafe-inline'`). With `script-src 'self'` it is blocked, the stylesheet never switches off `media="print"`, and the whole app renders unstyled. The earlier "safe today" note was wrong — verified broken in prod 2026-06-25. **Validate the new `initial` ceiling against a real prod build first**; consider removing the unused `@angular/material` dep as a cheap win|`frontend/angular.json`, `frontend/package.json`|S|P1|
